...
const App: React.FC = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    ...
    const saved = loadLayoutFromStorage(LAYOUT_STORAGE_KEY);
    return saved?.isDarkMode ?? false;
  });
  ...
  useEffect(() => {
    ...
    const rawHash = typeof window !== 'undefined' ? window.location.hash.slice(1).trim() : '';
    if (rawHash) {
      try {
        const decoded = decodeURIComponent(escape(atob(rawHash)));
        const parsed = parseImportPayload(JSON.parse(decoded));
        if (parsed) {
          ...
          loadedFromHash = true;
          pushToast('Shared diagram loaded from link.', 'success');
        }
      } catch {
        pushToast('Shared link could not be loaded. Falling back to saved workspace.', 'warning');
      } finally {
    ...
    hasLoadedFromStorage.current = true;
  }, [applyLayoutSettings, applySnapshot, pushToast]);
  ...
  useEffect(() => {
    ...
    saveDiagramTimeoutRef.current = window.setTimeout(() => {
      ...
      if (!saved) {
        setStorageWarning('Diagram could not be saved — storage may be full. Export your diagram to avoid losing work.');
      } else {
        setStorageWarning(null);
      }
    }, 180);
  ...
  const handleCopySelection = useCallback(() => {
    ...
    clipboardEdgesRef.current = edges
      .filter((edge) => selectedNodeSet.has(edge.sourceId) && selectedNodeSet.has(edge.targetId))
      .map((edge) => ({ ...edge }));
  ...
  const handlePasteSelection = useCallback(() => {
    ...
    const idMap = new Map<string, string>();
    const duplicatedNodes = clipboardRef.current.map((node) => {
      const newId = createId('node');
      idMap.set(node.id, newId);
      const nextPosition = {
        x: node.position.x + 40,
    ...
    const duplicatedEdges = clipboardEdgesRef.current
  ...
  const handleImportDiagram = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    try {
      ...
      pushToast('Unable to copy link automatically. Copy it from the address bar.', 'warning');
    }
  ...
  useEffect(() => {
    const handleShortcut = (event: KeyboardEvent) => {
      ...
      if (isMetaOrCtrl && key === 'c' && selectedNodeIds.length > 0) {
        event.preventDefault();
        handleCopySelection();
        return;
      }

      if (isMetaOrCtrl && key === 'v' && clipboardRef.current.length > 0) {
        event.preventDefault();
  ...
  return (
    <div className={`finflow-app-shell flex h-screen flex-col overflow-hidden ${isDarkMode ? 'dark text-slate-100' : 'text-slate-900'}`}>
      ...
      <ConfirmModal
        ...
        isDarkMode={isDarkMode}
        title="Reset canvas to starter template?"
        message="This will replace your current canvas. A recovery snapshot will be saved first so you can restore it."
      ...
      <ConfirmModal
        ...
        message="Importing will replace your current canvas. A recovery snapshot will be saved first."
